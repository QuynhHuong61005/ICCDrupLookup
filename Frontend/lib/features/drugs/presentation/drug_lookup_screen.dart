...
class _DrugLookupScreenState extends ConsumerState<DrugLookupScreen> {
  ...
  void _onIngredientChanged(String? ingredient) {
    ref.read(drugSearchProvider.notifier).search(_searchController.text, ingredient: ingredient ?? '');
  }
          ...
          AppCard(
            child: Column(
              ...
              children: [
                ...
                AppSpacing.gapH16,
                ref.watch(drugIngredientsProvider).when(
                  data: (ingredients) {
                    return DropdownButtonFormField<String>(
                      decoration: const InputDecoration(
                        labelText: 'Filter by Active Ingredient',
                        border: OutlineInputBorder(),
                        contentPadding: EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                      ),
                      value: state.ingredient.isEmpty ? null : state.ingredient,
                      items: [
                        const DropdownMenuItem(value: '', child: Text('All Ingredients')),
                        ...ingredients.map((i) => DropdownMenuItem(value: i, child: Text(i))),
                      ],
                      onChanged: _onIngredientChanged,
                    );
                  },
                  loading: () => const LinearProgressIndicator(),
                  error: (e, st) => const Text('Failed to load active ingredients'),
                ),
              ],
            ),
          ),
