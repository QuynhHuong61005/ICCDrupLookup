...
import 'package:medprescribe_frontend/features/auth/presentation/register_screen.dart';
...
import 'package:medprescribe_frontend/features/dashboard/presentation/profile_screen.dart';
  ...
  static GoRouter createRouter(ProviderContainer container) {
    return GoRouter(
      ...
      redirect: (context, state) {
        ...
        // Not authenticated → send to login (unless already there, on register, or on OTP)
        if (!isAuth && !isOnLoginPage && !isOnRegisterPage && !isOnOtpPage) {
          return login;
        }

        // Already authenticated and on login or register → go to dashboard
        // (but NOT if on OTP — the OTP screen handles its own redirect)
        if (isAuth && (isOnLoginPage || isOnRegisterPage)) {
          return dashboard;
        }
